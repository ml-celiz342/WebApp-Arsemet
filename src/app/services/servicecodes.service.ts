...
export class ServicecodesService {
  private apiURLServiceCodes = environment.apiUrl + 'servicecodes';

  constructor(private http: HttpClient) {}

  getServiceCodes(): Observable<Servicio[]> {
    const baseUrl = `${this.apiURLServiceCodes}?limit=100`;
    return this.http.get<any>(baseUrl).pipe(
      ...
      reduce<Servicio[], any>((acc, response) => {
        if (
          response &&
  ...
  createServiceCode(nombre: string) {
    const baseUrl = `${this.apiURLServiceCodes}`;
    const body = {
      name: nombre,
    };
  ...
  deleteServiceCode(id: number): Observable<number> {
    const baseUrl = `${this.apiURLServiceCodes}/${id}`;
    return this.http
